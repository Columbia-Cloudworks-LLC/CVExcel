...
function Extract-MSRCData {
    <#
    .SYNOPSIS
        Enhanced data extraction function for MSRC and other advisory pages.
        
    .DESCRIPTION
        Extracts CVE data, download links, patch information, and other relevant
        details from advisory page HTML content.
    #>
    param(
        [Parameter(Mandatory)]
        [string]$HtmlContent,
    ...
    Write-Log -Message "Extracting data from advisory page..." -Level "INFO"
    ...
    if ($kbMatches.Count -gt 0) {
        ...
        Write-Log -Message "Found KB articles: $($kbList -join ', ')" -Level "SUCCESS"
    }

    # Extract affected versions (simple pattern)
    if ($HtmlContent -match 'Affected [Pp]roducts?[\s:]+([^\r\n<]+)') {
        $result.AffectedVersions = $matches[1].Trim()
        Write-Log -Message "Found affected versions: $($result.AffectedVersions)" -Level "SUCCESS"
    }
